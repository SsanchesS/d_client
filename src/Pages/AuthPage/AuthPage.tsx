...
const onClick_auth = async()=>{
  ...
  try{
    ...
    const data = await auth(user).unwrap()
    if(data.code >= 400){
...
return (
  ...
  <div className={`card ${s.center}`}>
    <div className={s.AuthPage}>
      ...
      <h2 className={ `${s.text} white mb10px`}>Тестовые аккаунты (пароль как почта):</h2>
      <h2 className={ `${s.text} white mb10px`}>admin@test.ru</h2>
      <h2 className={ `${s.text} white mb10px`}>user@test.ru</h2>
